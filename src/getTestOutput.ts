...
function getTestOutput(fileName: string): Promise<string> {
  return importGraph(fileName)
    .then((graph: Array<string>) => {
      ...
      return outputs.reduce((aggOutput, output, index) =>
        `${aggOutput}Output #${index + 1}: ${output}\n`, '');
    });
}
